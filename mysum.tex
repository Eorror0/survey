...
Traditional survival analysis involves predicting the duration before a specific event occurs, which assumes that the duration follows an unknown distribution. Among popular modeling methods, the Cox proportional risk model is widely used. This model focuses on hazard modeling rather than survival functions:

$h(t|x) = h_0(t) \exp(\beta^Tx)$

where:
\begin{align*}
& h(t|x) \text{ is the hazard function at time } t \text{ given covariates } x, \\
& h_0(t) \text{ is the baseline hazard function, representing the hazard when } x = 0, \\
& \beta \text{ is a vector of coefficients,} \\
& x \text{ is a vector of covariates.}
\end{align*}


$\begin{aligned}l(\beta)&=-\sum_{i=1}^n\delta_i\left(\beta^Tx_i-log\sum_{j\in R(t_i)}exp(\beta^Tx_j)\right),\end{aligned}$
...
% Surformer
This study proposes an innovative MIL neural network termed pattern-perceptive survival transformer (Surformer) \cite{atabansi2023survey}for WSI-based survival analysis. Briefly, Surformer comprises three components: a ratio-reserved cross-attention module (RRCA), a multi-head selfattention module (MHSA) , and a multi-head cross-attention module (MHCA), RRCA is the key module in Surformer. In the RRCA process, both pgloabals and plocals function as detectors to gather correlative features from WSIs.Here, the pgloabal is similar to the classification token of Bert\cite{devlin2018bert} and targets to aggregate all valuable features for survival prediction.RRCA simultaneously detects global features and multiple pattern-specific local features through a learnable global prototype pgloabal and multiple local prototypes plocal and quantifies the patches correlative to each plocals in the form of ratio factors (rfs). The ratio information is subsequentially embedded into the feature space for representation enhancement. 
...
Graphs are widely used to represent topological structures. However, modeling WSI as a graph is not simple. Cell-graph [6] are not feasible for WSI because they have a large number of units and many possible noise nodes. Therefore, most of the current work is based on WSI's patches to build graph models.
